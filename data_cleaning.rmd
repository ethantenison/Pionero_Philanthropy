...
data$crime[data$crime == "Y" ] <- "Security"
...
library(stringr)
        ...
        data <- dplyr::select(data, npo,category,latitude, longitude, address, municipality, department, budget, budget_adj, size,
                       year_founded,npo_age, partner_status, constant_size,constant_color, everything())
...
to_paste <- demographics %>% select(Department, Population, Poverty.Rate, Total.Literacy.Rate, Homicide.Rate, Total.Employment.Rate, Improved.Sanitation.Access, Gross.Birth.Rate)
...
for (i in 1:length(demographic_map$measure)){
                ...
                # else if(demographic_map$measure[i] == "Number.of.Preprimary.School.Students"){        
                #                 demographic_map$units[i] <- "students"
                # }else if(demographic_map$measure[i] == "Number.of.Primary.School.Students"){        
                #                 demographic_map$units[i] <- "students"
                # }else if(demographic_map$measure[i] == "Number.of.Middle.School.Students"){        
                #                 demographic_map$units[i] <- "students"
                # }else if(demographic_map$measure[i] == "Number.of.High.School.Students"){        
                #                 demographic_map$units[i] <- "students"
                #               }
}
