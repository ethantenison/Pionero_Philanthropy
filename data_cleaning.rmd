...
data <- googlesheets4::read_sheet(ss= "https://docs.google.com/spreadsheets/d/1klJ0aUZF1HX8qwR8EjsOhWUB9EJH6PYYdzPl2fQUvuM/edit?usp=sharing", sheet = "MD", col_names = TRUE, col_types = "ccncccccnncccnncccnncccnncccnncccnccccccccccccccccccccc") # Here I had to set guess_max = 5 because it was reading in the longitude column as a list
...
data <- data %>% tidyr::unite(list_categories,health:animal_welfare, remove = FALSE, sep =", ",na.rm = TRUE)
...
data <- data %>% tidyr::unite(tax_details, us_tax_status:guate_reg, remove = FALSE, sep =", ",na.rm = TRUE)
...
library(stringr)
        ...
        data$total_eval_score[data$total_eval_score == "TBE"] <- "To be evaluated"
         #data$total_eval_score_adj <- data$total_eval_score_adj + ifelse(data$partner_status == "Partnered", 2, 0)
        ...
        data$efficiency[data$efficiency == "TBE"] <- "To be evaluated"
         #data$efficiency_adj <- data$efficiency_adj + ifelse(data$partner_status == "Partnered", 1, 0)
        ...
        data$transparency[data$transparency == "TBE"] <- "To be evaluated"
         #data$transparency_adj <- data$transparency_adj + ifelse(data$partner_status == "Partnered", 1, 0)
        ...
        data$relevance[data$relevance == "TBE"] <- "To be evaluated"
         #data$relevance_adj <- data$relevance_adj + ifelse(data$partner_status == "Partnered", 1, 0)
        ...
        data$impact[data$impact == "TBE"] <- "To be evaluated"
         #data$impact_adj <- data$impact_adj + ifelse(data$partner_status == "Partnered",1, 0)
       ...
       # data$sustainability_adj <- data$sustainability_adj + ifelse(data$partner_status == "Partnered", 1, 0)
...
demographics <- demographics %>% mutate("None Selected"= NA) %>% dplyr::select(-c(X,Number.of.Preprimary.School.Students, Number.of.Primary.School.Students, Number.of.Middle.School.Students,Number.of.High.School.Students )) %>% mutate(Number.of.Libraries = Number.of.Libraries/(Population/100000))
