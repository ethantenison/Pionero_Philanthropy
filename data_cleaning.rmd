...
data$religious_aff[is.na(data$religious_aff)] <- "None"

data <- data %>% unite(list_categories,health:religious_aff, remove = FALSE, sep =", ",na.rm = TRUE)
...
demographics <- demographics %>% mutate(same=1) %>%  dplyr::select(-c(X))
demographics <- gather(demographics, measure, value, population:same)
...
library(stringr)
demographic_map$measure <- str_replace_all(demographic_map$measure, "_", " ")
demographic_map$measure <- str_to_title(demographic_map$measure)
