...
data <- read_sheet(ss= "https://docs.google.com/spreadsheets/d/1klJ0aUZF1HX8qwR8EjsOhWUB9EJH6PYYdzPl2fQUvuM/edit?usp=sharing", sheet = "MD", col_names = TRUE, guess_max = 353) # Here I had to set guess_max = 5 because it was reading in the longitude column as a list
...
df <- read.csv("data/data.csv", fileEncoding = "UTF-8", encoding = "UTF-8", stringsAsFactors = FALSE, colClasses=c("guate_govt_funding"="character", "seal_excellence"="character"))
...
data$eligibility[data$eligibility == "Y"] <- "None"
data$eligibility[data$eligibility == "E"] <- "None"
data$eligibility[data$eligibility == "NC"] <- "Not eligible because nonprofit has closed"
data$eligibility[data$eligibility == "NBL"] <- "Exceeds budget limit"
data$eligibility[data$eligibility == "NPI"] <- "No public online information available"
data$eligibility[data$eligibility == "NRA"] <- "Recent activity exceeds parameters"
data$eligibility[is.na(data$eligibility)] <- "Unknown"
...
library(stringr)
        ...
        data <- mutate(data, constant_color = NA)
                
        data <- dplyr::select(data, npo,category,latitude, longitude, address, municipality, department, budget, budget_adj, size,
                       year_founded,npo_age, partner_status,eligibility, constant_size,constant_color, everything())
...
demographics <- demographics %>% mutate("Nothing Selected"=1) %>% dplyr::select(-c(X,Number.of.Preprimary.School.Students, Number.of.Primary.School.Students, Number.of.Middle.School.Students,Number.of.High.School.Students )) %>% mutate(Number.of.Libraries = Number.of.Libraries/(Population/100000))
