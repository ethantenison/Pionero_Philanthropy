...
data <- read_sheet(ss= "https://docs.google.com/spreadsheets/d/1klJ0aUZF1HX8qwR8EjsOhWUB9EJH6PYYdzPl2fQUvuM/edit?usp=sharing", sheet = "Sheet1", col_names = TRUE)
...
data <- bind_rows(data_1st_add,data_2nd_add,data_3rd_add,data_4th_add, data_5th_add)
...
```{r clickable_website}
 data$website <- paste0("<a href=\"",data$website,"\">",data$npo,"</a>")
...
#Color for size 
                data <- mutate(data, sizecolor = NA)
                
                
                for (i in 1:length(data$size)){
                        if(is.na(data$size[i])){
                                data$sizecolor[i] <-"#E68310"
                        }
                        else if(data$size[i] == "Small"){        
                                data$sizecolor[i] <- "#7F3C8D"               
                        }
                        else if(data$size[i] == "Medium"){        
                                data$sizecolor[i] <- "#11A579"              
                        }
                        else if(data$size[i] == "Large"){        
                                data$sizecolor[i] <- "#3969AC"              
                        }
                        else if(data$size[i] =="Micro"){        
                                data$sizecolor[i] <- "#F2B701"              
                        }
                        else if(data$size[i] == "Nano"){        
                                data$sizecolor[i] <- "#E73F74"  
                        }
                        else if(data$size[i] == "Very Large"){        
                                data$sizecolor[i] <- "#80BA5A"              
                        }
...
#color palette for continous #f7feae,#045275
        ...
        data <- filter(data, year_founded != "NA")
                ii <- cut(data$year_founded, breaks = seq(min(data$year_founded), max(data$year_founded), len = 100),
                          include.lowest = TRUE)
                
                data$year_founded_color <- colorRampPalette(c("#f7feae", "#045275"))(99)[ii]
...
#color for partner status 
                data <- mutate(data, partner_status_color = NA)

                for (i in 1:length(data$partner_status)){
                        if(data$partner_status[i] == "P"){        
                                data$partner_status_color[i] <- "#7F3C8D"               
                        }
                        else if(data$partner_status[i] == "E"){        
                                data$partner_status_color[i] <- "#11A579"              
                        }
                        else if(data$partner_status[i] == "DP"){        
                                data$partner_status_color[i] <- "#3969AC"              
                        }
                        else if(data$partner_status[i] == "NE"){        
                                data$partner_status_color[i] <- "#F2B701"              
                        }
                }
...
#Altering the size variable 
        ...
        data <- dplyr::select(data, npo,category,latitude, longitude, address, municipality, department, budget, size, sizecolor,
                       year_founded,year_founded_color, partner_status, partner_status_color, constant, constant_color, 
                       everything())
